...
import { columns } from "../app/todos/columns";
...
export default async function IndexPage() {
  ...
  // fully hydrating todo slice
  const fullHydrateSlices = {
    todo: state.todo
  };

  // fully hydrating todo & product slice which is equal to allSlice as we have only two slices [todo & product]
  const fullHydrateTodoAndProduct = {
    todo: state.todo,
    product: state.product
  };

  // partially hydrating todo slice
  ...
  return (
    <NextReduxWrapper hydrateStates={fullHydrateSlices}>
      <DataTable columns={columns} data={state.todo.data.todos.slice(10, 60)} />
    </NextReduxWrapper>
  );
}
